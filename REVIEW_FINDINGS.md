...
| iCalendar DNS validation was separate from the request connection | Resolved in 0.5.1: each request and redirect resolves and validates public addresses, connects the socket directly to the selected validated IP, and verifies TLS for the original hostname. |
...
All listed safeguards remain implemented. The exact 0.5.1 tagged commit must
